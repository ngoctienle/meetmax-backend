...
import { ICommentDocument, ICommentNameList } from '@commentFeatures/interfaces/comment.interface'
...
const log: Logger = environment.createLogger('commentsCache')
...
export class CommentCache extends BaseCache {
  ...
  public async savePostCommentToCache(postId: string, value: string): Promise<void> {
    try {
      ...
      let count: number = Utils.parseJson(commentsCount[0]) as number
      count += 1
  ...
  public async getCommentsFromCache(postId: string): Promise<ICommentDocument[]> {
    try {
      ...
      for (const item of reply) {
        list.push(Utils.parseJson(item))
      }
  ...
  public async getCommentsNamesFromCache(postId: string): Promise<ICommentNameList[]> {
    try {
      ...
      for (const item of comments) {
        const comment: ICommentDocument = Utils.parseJson(item) as ICommentDocument
